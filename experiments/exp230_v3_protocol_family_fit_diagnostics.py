"""Fit diagnostics for protocol-invariant v3 manifests."""
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V3 protocol fit diagnostics.")
    parser.add_argument(
    ...
    return ExperimentConfig(
        args.manifest_dir, args.dims, args.steps, args.restarts, args.seed
    )


def main() -> None:
    config = parse_args()
    rows = compute_v3_protocol_manifest_family_fit_rows(
        config.manifest_dir,
    ...
    summary = summarize_v3_protocol_fit_rows(rows)
    fit_path = write_csv(
        rows,
        Path("outputs/data/v3_protocol_manifest_family_fit_comparison.csv"),
        ["manifest_id", "family_name", "family_kind"],
    )
    summary_path = write_csv(
        summary,
        Path("outputs/data/v3_protocol_manifest_family_fit_summary.csv"),
        ["family_name", "family_kind", "embedding_dim"],
    )
    save_family_metric_figure(
        summary,
        metric="mean_heldout_violation",
        path=Path("outputs/figures/v3_protocol_manifest_family_heldout_violation.png"),
        ylabel="Mean heldout violation",
    )
    save_family_metric_figure(
        summary,
        metric="mean_generalization_gap",
        path=Path("outputs/figures/v3_protocol_manifest_family_generalization_gap.png"),
        ylabel="Mean generalization gap",
    )
    print(f"Wrote v3 protocol fit diagnostics: {fit_path}, {summary_path}")
