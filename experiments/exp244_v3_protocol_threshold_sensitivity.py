"""V3 protocol threshold sensitivity without retuning."""
...
from causal_spacetime_lab.state_change_manifest_v3_protocol_threshold_sensitivity import (  # noqa: E501
    v3_protocol_threshold_sensitivity_rows,
)
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V3 protocol threshold sensitivity.")
...
def run_experiment(config: ExperimentConfig) -> list[dict[str, float | str]]:
    ...
    preconditions = evaluate_v3_protocol_preconditions(
        config.output_dir / "manifests_v3",
        bundle,
    )
    return v3_protocol_threshold_sensitivity_rows(
        metrics,
        preconditions,
...
def main() -> None:
    ...
    path = write_csv(
        rows,
        data_path(config.output_dir, "v3_protocol_threshold_sensitivity.csv"),
        [
            "heldout_threshold",
            "null_gap_threshold",
    ...
    figure = save_metric_bar(
        ...
        path=config.output_dir / "figures" / "v3_protocol_threshold_sensitivity.png",
        ylabel="carry-forward count",
    )
    print(f"Wrote v3 protocol threshold sensitivity: {path}")
