"""Audit v4 protocol metadata/provenance carry-forward preconditions."""
...
from causal_spacetime_lab.state_change_manifest_v4_preconditions import (
    ...
    v4_protocol_precondition_report_to_row,
    v4_protocol_precondition_summary,
)
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V4 protocol precondition audit.")
...
def run_experiment(
    ...
    reports = evaluate_v4_protocol_preconditions(config.manifest_dir, bundle)
    rows = [v4_protocol_precondition_report_to_row(report) for report in reports]
    return rows, v4_protocol_precondition_summary(rows)
...
def main() -> None:
    ...
    summary_path = write_csv(
        summary,
        data_path(config.output_dir, "v4_protocol_precondition_summary.csv"),
        ["family_kind", "status", "count"],
    )
    print(f"Wrote v4 protocol precondition audit: {path}, {summary_path}")
