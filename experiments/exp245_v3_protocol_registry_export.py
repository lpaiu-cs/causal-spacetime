"""Export the v3 protocol carry-forward registry."""
...
from causal_spacetime_lab.state_change_manifest_v3_protocol_registry import (
    ...
    v3_protocol_registry_summary_rows,
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="Export v3 protocol registry.")
...
def run_experiment(
    ...
    if not decisions:
        return None, [
            {
                "registry_id": "",
                "criteria_name": "fixed_m34_v3_protocol",
                "family_name": "",
                "family_kind": "",
                ...
                "failed_reasons": "missing_v3_protocol_decisions",
                "warning_reasons": "",
    ...
    return (
        write_v3_protocol_carry_forward_registry(registry, config.output_dir),
        v3_protocol_registry_summary_rows(registry),
    )
...
def main() -> None:
    ...
    path = write_csv(
        rows,
        data_path(config.output_dir, "v3_protocol_carry_forward_registry_export.csv"),
        ["registry_id", "criteria_name", "family_name", "decision"],
    )
    print(f"Wrote v3 protocol registry summary: {path}")
    if registry_path is not None:
        print(f"Wrote v3 protocol registry: {registry_path}")
