"""Restart and latent-order stability diagnostics for v4 protocol manifests."""
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V4 protocol stability diagnostics.")
    parser.add_argument(
        "--manifest-dir",
        type=Path,
...
def main() -> None:
    ...
    restart_path = write_csv(
        restart_rows,
        Path("outputs/data/v4_protocol_manifest_family_restart_stability.csv"),
        ["manifest_id", "family_name", "restart_std"],
    )
    latent_path = write_csv(
        latent_rows,
        Path("outputs/data/v4_protocol_manifest_family_latent_order_stability.csv"),
        ["manifest_id", "family_name", "latent_order_disagreement"],
    )
    save_family_metric_figure(
        restart_rows,
        metric="restart_std",
        path=Path("outputs/figures/v4_protocol_manifest_restart_std.png"),
        ylabel="Restart std",
    )
    save_family_metric_figure(
        latent_rows,
        metric="latent_order_disagreement",
        path=Path("outputs/figures/v4_protocol_manifest_latent_order_disagreement.png"),
        ylabel="Latent-order disagreement",
    )
    print(f"Wrote v4 protocol stability diagnostics: {restart_path}, {latent_path}")
