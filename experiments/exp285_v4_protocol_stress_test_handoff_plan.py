"""Generate the v4 protocol future stress-test handoff plan."""
...
from causal_spacetime_lab.state_change_manifest_v4_stress_handoff import (
    ...
    v4_protocol_stress_test_plan_rows,
)
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V4 protocol stress-test plan.")
    parser.add_argument("--output-dir", type=Path, default=Path("outputs"))
    return ExperimentConfig(parser.parse_args().output_dir)


def run_experiment(config: ExperimentConfig) -> list[dict[str, float | str]]:
    registry = read_v4_protocol_registry(config.output_dir)
    if registry is None:
        return [
            {
                ...
                "allowed": 0.0,
                "allowed_tests": "",
                "required_caveats": "missing_v4_protocol_registry",
                "stop_if_failed": 1.0,
            }
        ]
    entries = build_v4_protocol_stress_test_plan(registry)
    rows = v4_protocol_stress_test_plan_rows(entries)
    for row in rows:
...
def main() -> None:
    ...
    path = write_csv(
        ...
        data_path(config.output_dir, "v4_protocol_stress_test_handoff_plan.csv"),
        ["row_type", "family_name", "decision", "allowed"],
    )
    print(f"Wrote v4 protocol stress-test handoff plan: {path}")
