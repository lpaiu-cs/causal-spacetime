"""The O5 Poisson-count campaign's seed reservation, in its own ref
namespace.
...
What is new here is only the namespace and the retained set:
  ...
  * this campaign claims `refs/o5/reservation`;
  * ALL THREE prior reservations are retained permanently and verified
    before any claim -- `refs/o4/reservation` (the O4 abort's claim on
    retired streams), `refs/o4b/reservation` (the O4b campaign's), and
    `refs/o5pilot/reservation` (the ambiguity pilot's, whose FEASIBLE
    result this campaign's power sentence rests on). A cleared ref
...
def probe_namespace() -> None:
    """Prove at PREFLIGHT that `refs/o5/` is writable: a NEW
    ...
    obj = _make_commit("o5 preflight probe")
    push = subprocess.run(
