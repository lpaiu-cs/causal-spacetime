"""The S6 M=3.0 count campaign's seed reservation, in its own ref
namespace.
...
What is new here is only the namespace and the retained set:
  ...
  * this campaign claims `refs/s6m30/reservation`;
  * ALL SEVEN prior reservations are retained permanently and
    verified before any claim -- refs/o4, refs/o4b, refs/o5pilot,
    refs/o5, refs/s6m14pilot, refs/s6m14 and refs/s6m30pilot (every
    executed or in-flight claim on drawn streams). A cleared ref
...
def probe_namespace() -> None:
    """Prove at PREFLIGHT that `refs/s6m30/` is writable: a NEW
    ...
    obj = _make_commit("s6m30 preflight probe")
    push = subprocess.run(
