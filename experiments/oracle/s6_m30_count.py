"""S6 M=3.0 Poisson-count campaign -- the frozen runner (freeze, NO results).
...
the ambiguity pilot (docs/prereg/p14_s6_m30_pilot.json, FEASIBLE at
...
alternative): acceptance [52,986, 54,220] on K_true; worst endpoint
power 0.931493 at V_hi; at U = 30 the guaranteed window keeps
0.912741; joint completion-and-concordance
    >= 0.912741 - 0.001 (pilot tail budget) - 0.010 (pilot alpha)
       - exp(-81,307)  (exact Chernoff on P(2N > 11,000,000))
    =  0.901741 >= 0.90.
...
BUDGET. max_calls = 11,000,000 + 6,466: the scan's hard cap (P(2N >
cap) <= exp(-81,307) under the alternative) plus the rung-measured
deterministic full-wrapper G3a preflight, which runs BEFORE the
reservation and is charged to the same budget. E[calls] <= 2 * A *
SCALE_rung = 9,215,381 (~2.3 h at the projected price; less at the
...
reservation claim on `refs/s6m30/reservation` (the claim RETURNS its
...
clean exact checkout of the freeze branch head):
    ...
    python experiments/oracle/s6_m30_count.py --preflight \
        --freeze-rev <full 40-hex freeze branch head>
    python experiments/oracle/s6_m30_count.py \
        --freeze-rev <full 40-hex freeze branch head>
"""
...
import s6_m30_count_reservation as reservation  # noqa: E402
...
_MANIFEST = (_REPO / "docs" / "prereg"
             / "p14_s6_m30_count_freeze_manifest.json")
_ARTIFACT = _REPO / "docs" / "prereg" / "p14_s6_m30_count.json"
_INCIDENT = _REPO / "docs" / "prereg" / "p14_s6_m30_count_incident.json"
_CHECKPOINT = (_REPO / "docs" / "prereg"
               / "p14_s6_m30_count_checkpoint.json")
...
G3A_PREFLIGHT_CALLS = 6_466

#: The frozen configuration. The oracle endpoints are the ACTUAL
#: rung artifact values (docs/prereg/p14_s6_m30_volume.json,
...
#: window.
ACCEPTANCE = (52_986, 54_220)

#: The rung geometry, frozen as literals and re-derived at import
#: through the one exact path (the ladder discipline).
_M = 3.0
_RUNG = {
    "dt": 12.442423039673733,
    "r_lo": 11.442907037276841,
    "r_hi": 18.769059682298728,
    "psi_max": 0.39909090507371286,
    "scale": 10472.024224793164,
}
_g = s6.rung_geometry(_M)
if any(_g[k] != v for k, v in _RUNG.items()):
    raise SystemExit(
        f"s6_m30_count: frozen rung literals drifted from the one "
...
def _import_gate() -> None:
    ...
    if (r["v_lo"] != FROZEN["v_lo"] or r["v_hi"] != FROZEN["v_hi"]
        ...
        raise SystemExit(
            f"s6_m30_count: the frozen oracle endpoints "
    ...
    if v_ref != FROZEN["v_ref"]:
        raise SystemExit(
            f"s6_m30_count: frozen v_ref {FROZEN['v_ref']} is "
    ...
    if (d["verdict"] != "FEASIBLE"
        ...
        raise SystemExit(
            f"s6_m30_count: the ambiguity pilot's decision "
...
def main() -> None:
    ...
    try:
        ...
        result = {
            "kind": "results",
            "run_kind": "s6_m30_count",
            "stage": ("S6 M=3.0 Poisson-count campaign: sprinkled "
...
PROTOCOL_SURFACE = (
    "experiments/oracle/s6_m30_count.py",
    "experiments/oracle/s6_m30_count_reservation.py",
    ...
    "docs/prereg/p14_s6_m30_count.md",
    "docs/prereg/p14_s6_m30_volume.json",
...
def build_manifest() -> dict:
    return {
        "stage": ("S6 M=3.0 Poisson-count campaign freeze manifest "
