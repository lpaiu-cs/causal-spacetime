"""The S6 M=1.4 ambiguity pilot's seed reservation, in its own ref
namespace.
...
What is new here is only the namespace and the retained set:
  ...
  * this pilot claims `refs/s6m14pilot/reservation`;
  * ALL FOUR prior reservations are retained permanently and verified
    before any claim -- `refs/o4/reservation`, `refs/o4b/reservation`,
    `refs/o5pilot/reservation` and `refs/o5/reservation` (the executed
    campaigns' claims on retired streams). A cleared ref
...
#: This campaign's ref.
REF = "refs/s6m14pilot/reservation"
...
_IDENTITY = "s6m14pilot-reservation"
...
def probe_namespace() -> None:
    """Prove at PREFLIGHT that `refs/s6m14pilot/` is writable: a NEW
    ...
    obj = _make_commit("s6m14pilot preflight probe")
    push = subprocess.run(
