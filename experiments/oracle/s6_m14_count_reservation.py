"""The S6 M=1.4 count campaign's seed reservation, in its own ref
namespace.
...
What is new here is only the namespace and the retained set:
  ...
  * this campaign claims `refs/s6m14/reservation`;
  * ALL FIVE prior reservations are retained permanently and verified
    before any claim -- `refs/o4/reservation`, `refs/o4b/reservation`,
    `refs/o5pilot/reservation`, `refs/o5/reservation` and
    `refs/s6m14pilot/reservation` (the executed campaigns' and pilots'
    claims on retired streams). A cleared ref
...
def probe_namespace() -> None:
    """Prove at PREFLIGHT that `refs/s6m14/` is writable: a NEW
    ...
    obj = _make_commit("s6m14 preflight probe")
    push = subprocess.run(
...
def claim(payload: dict) -> str:
    """Claim the campaign stream globally, before the first draw, and
    ...
    if (already := held()) is not None:
        raise SystemExit(
            ...
            f"campaign has already opened this stream from some "
