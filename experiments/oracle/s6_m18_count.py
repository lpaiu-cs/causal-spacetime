"""S6 M=1.8 Poisson-count campaign -- the frozen runner (freeze, NO results).
...
the ambiguity pilot (docs/prereg/p14_s6_m18_pilot.json, FEASIBLE at
k = 0): P(U > 30) <= 1e-3 at 99%
confidence under the PILOT's
...
alternative): acceptance [52,905, 54,137] on K_true; worst endpoint
power 0.931336 at V_hi; at U = 30 the guaranteed window keeps
0.912536; joint completion-and-concordance
    >= 0.912536 - 0.001 (pilot tail budget) - 0.010 (pilot alpha)
       - exp(-131,245)  (exact Chernoff on P(2N > 23,000,000))
    =  0.901536 >= 0.90.
...
BUDGET. max_calls = 23,000,000 + 6,461: the scan's hard cap (P(2N >
cap) <= exp(-131,245) under the alternative) plus the rung-measured
deterministic full-wrapper G3a preflight, which runs BEFORE the
reservation and is charged to the same budget. E[calls] <= 2 * A *
SCALE_rung = 19,697,895 (~5.0 h at the projected price; less at the
observed early-exit rate -- far under the 24 h wall). The caps are frozen: RAISING THEM
...
reservation claim on `refs/s6m18/reservation` (the claim RETURNS its
...
clean exact checkout of the freeze branch head):
    ...
    python experiments/oracle/s6_m18_count.py --preflight \
        --freeze-rev <full 40-hex freeze branch head>
    python experiments/oracle/s6_m18_count.py \
        --freeze-rev <full 40-hex freeze branch head>
"""
...
import s6_m18_count_reservation as reservation  # noqa: E402
...
_MANIFEST = (_REPO / "docs" / "prereg"
             / "p14_s6_m18_count_freeze_manifest.json")
_ARTIFACT = _REPO / "docs" / "prereg" / "p14_s6_m18_count.json"
_INCIDENT = _REPO / "docs" / "prereg" / "p14_s6_m18_count_incident.json"
_CHECKPOINT = (_REPO / "docs" / "prereg"
               / "p14_s6_m18_count_checkpoint.json")
...
#: rung artifact values (docs/prereg/p14_s6_m18_volume.json,
#: target-met at ratio 0.0049996), adopted verbatim; v_ref is that
#: artifact's recommendation (the standalone midpoint), whose
#: adoption PR #98 explicitly deferred to THIS freeze. Caps are part of
#: the freeze: no auto-raise, ever.
FROZEN = {
    "a_intensity": 720.0,
    "tau": 0.025,
    "alpha": 0.05,
    "v_lo": 73.96863670309186,
    "v_hi": 74.7119200322603,
    "v_ref": 74.34027836767608,
    "u_power_ceiling": 30,
    "seed_name": "s6_m18_count",
    "tol": s1.DEFAULT_TOL,
    "chunk": 65_536,
    "scan_call_cap": 23_000_000,
    "max_calls": 23_006_461,
    "max_wall_s": 86_400.0,
}
...
#: window.
ACCEPTANCE = (52_905, 54_137)

#: The rung geometry, frozen as literals and re-derived at import
#: through the one exact path (the ladder discipline).
_M = 1.8
_RUNG = {
...
if any(_g[k] != v for k, v in _RUNG.items()):
    raise SystemExit(
        f"s6_m18_count: frozen rung literals drifted from the one "
        f"exact path: {_RUNG} != derived")
_GEOMETRY = _g
del _g

#: The anchors, from the ladder's single source of truth.
...
def _import_gate() -> None:
    ...
    o3p = json.loads((_REPO / "docs" / "prereg"
                      / "p14_s6_m18_volume.json").read_text(
                          encoding="utf-8"))
    r = o3p["result"]
    if (r["v_lo"] != FROZEN["v_lo"] or r["v_hi"] != FROZEN["v_hi"]
            or r["status"] != "target-met"):
        raise SystemExit(
            f"s6_m18_count: the frozen oracle endpoints "
            f"({FROZEN['v_lo']}, {FROZEN['v_hi']}) do not match the "
            f"rung artifact ({r['v_lo']}, {r['v_hi']}, {r['status']})")
    v_ref = o3p["v_ref_rung_recommendation"]["value"]
    if v_ref != FROZEN["v_ref"]:
        raise SystemExit(
            f"s6_m18_count: frozen v_ref {FROZEN['v_ref']} is "
            f"not the rung artifact's recommendation {v_ref}")
    pilot = json.loads((_REPO / "docs" / "prereg"
                        / "p14_s6_m18_pilot.json").read_text(
                            encoding="utf-8"))
    d = pilot["decision"]
    if (d["verdict"] != "FEASIBLE"
            or d["u_max"] != FROZEN["u_power_ceiling"]):
        raise SystemExit(
            f"s6_m18_count: the ambiguity pilot's decision "
...
def run_scan(rng, budget, cfg: dict, on_chunk=None,
    ...
    if live is None:
        live = {}
    n_total = int(rng.poisson(cfg["a_intensity"] * _RUNG["scale"]))
...
def main() -> None:
    ...
    try:
        ...
        result = {
            "kind": "results",
            "run_kind": "s6_m18_count",
            "stage": ("S6 M=1.8 Poisson-count campaign: sprinkled "
                      "tri-state membership count against the rung's "
                      "certified volume"),
...
MANIFEST_NOTE = (
    ...
    "reservation follows the corrected s6-pilot copy; the rung "
    "oracle artifact, the O3 artifact o4_sizing imports, and the "
    "rung pilot's FEASIBLE result are pinned because the frozen "
    "endpoints, the import surface and the U ceiling quote them. "
    "This manifest cannot certify itself: the run also demands the "
    "exact freeze branch head via --freeze-rev."
)

PROTOCOL_SURFACE = (
    "experiments/oracle/s6_m18_count.py",
    "experiments/oracle/s6_m18_count_reservation.py",
    ...
    "docs/prereg/p14_s6_m18_count.md",
    "docs/prereg/p14_s6_m18_volume.json",
    "docs/prereg/p14_s6_m18_pilot.json",
...
def build_manifest() -> dict:
    return {
        "stage": ("S6 M=1.8 Poisson-count campaign freeze manifest "
