"""S6 M=3.0 ambiguity pilot -- the frozen runner.

EXECUTION FOLLOWS THE STRONG-CURVATURE RULING (the PI approval
of A = 440, n = 1,843,669 and the seed allocation): after this freeze
...
WHAT THE PILOT MEASURES. One fresh stream (`s6_m30_pilot`) draws
n = 1,843,669 events from the SAME rung box measure the M = 3.0
count campaign will sprinkle (r^2 dr on the rung box, cos(psi)
uniform to the rung cap, phi, t in [0, dt_rung]). For each event the
full tri-state `causal_relation` at m = 3.0 is asked about both legs
...
BUDGET. max_calls = 2*n + 6,466: the hard per-event bound (two legs)
plus the rung-measured deterministic full-wrapper G3a preflight (at
m = 3.0 and the rung geometry -- NOT M = 1's 6,537), which runs BEFORE
...
claim on `refs/s6m30pilot/reservation` (the claim RETURNS its object; the
...
freeze branch head):
    ...
    python experiments/oracle/s6_m30_amb_pilot.py --preflight \
        --freeze-rev <full 40-hex freeze branch head>
    python experiments/oracle/s6_m30_amb_pilot.py \
        --freeze-rev <full 40-hex freeze branch head>
"""
...
import s6_m30_reservation as reservation  # noqa: E402
...
_MANIFEST = (_REPO / "docs" / "prereg"
             / "p14_s6_m30_pilot_freeze_manifest.json")
_ARTIFACT = _REPO / "docs" / "prereg" / "p14_s6_m30_pilot.json"
_INCIDENT = _REPO / "docs" / "prereg" / "p14_s6_m30_pilot_incident.json"
_CHECKPOINT = (_REPO / "docs" / "prereg"
               / "p14_s6_m30_pilot_checkpoint.json")
...
FROZEN = {
    ...
    "tail_budget": 0.001,
    "a_provisional": 440.0,
    "seed_name": "s6_m30_pilot",
    "tol": s1.DEFAULT_TOL,
    "chunk": 65_536,
    "max_calls": 3_693_804,         # = 2*n_events + G3A_PREFLIGHT_CALLS
    "max_wall_s": 86_400.0,
}
...
if (_g["dt"] != _RUNG["dt"] or _g["r_lo"] != _RUNG["r_lo"]
    ...
    raise SystemExit(
        f"s6_m30_pilot: frozen rung literals drifted from the one "
...
# PR #83 R1). The k = 1 boundary sits ~1.65e-8 of tail below the
# budget, while a double-precision binomial CDF at n ~ 2e6 carries an
# lgamma cancellation floor near 1e-8 relative -- comparable to this
# rung's own margin, so the same frozen inputs could flip the verdict across
...
def main() -> None:
    ...
    try:
        ...
        result = {
            "kind": "results",
            "run_kind": "s6_m30_pilot",
            "stage": ("S6 M=3.0 ambiguity pilot: fixed-n tri-state "
...
PROTOCOL_SURFACE = (
    "experiments/oracle/s6_m30_amb_pilot.py",
    "experiments/oracle/s6_m30_reservation.py",
    ...
    "docs/prereg/p14_s6_m30_pilot.md",
...
def build_manifest() -> dict:
    return {
        "stage": ("S6 M=3.0 ambiguity-pilot freeze manifest "
