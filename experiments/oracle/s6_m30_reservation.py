"""The S6 M=3.0 ambiguity pilot's seed reservation, in its own ref
namespace.
...
What is new here is only the namespace and the retained set:
  ...
  * this pilot claims `refs/s6m30pilot/reservation`;
  * ALL EIGHT prior reservations are retained permanently and
    verified before any claim -- refs/o4, refs/o4b, refs/o5pilot,
    refs/o5, refs/s6m14pilot, refs/s6m14, refs/s6m18pilot and
    refs/s6m18 (every executed or in-flight claim on drawn streams). A cleared ref
...
#: This campaign's ref.
REF = "refs/s6m30pilot/reservation"
...
_IDENTITY = "s6m30pilot-reservation"
...
def probe_namespace() -> None:
    """Prove at PREFLIGHT that `refs/s6m30pilot/` is writable: a NEW
    ...
    obj = _make_commit("s6m30pilot preflight probe")
    push = subprocess.run(
