"""S6 M=1.4 Poisson-count campaign -- the frozen runner (freeze, NO results).
...
the ambiguity pilot (docs/prereg/p14_s6_m14_pilot.json, FEASIBLE at
k = 1 -- the pre-sized boundary case): P(U > 30) <= 1e-3 at 99%
...
alternative): acceptance [53,045, 54,282] on K_true; worst endpoint
power 0.932156 at V_hi; at U = 30 the guaranteed window keeps
0.913550; joint completion-and-concordance
    >= 0.913550 - 0.001 (pilot tail budget) - 0.010 (pilot alpha)
       - exp(-120,283)  (exact Chernoff on P(2N > 34,000,000))
    =  0.902550 >= 0.90.
...
BUDGET. max_calls = 34,000,000 + 6,517: the scan's hard cap (P(2N >
cap) <= exp(-120,283) under the alternative) plus the rung-measured
deterministic full-wrapper G3a preflight, which runs BEFORE the
reservation and is charged to the same budget. E[calls] <= 2 * A *
SCALE_rung = 30,114,193 (~7.7 h at the projected price; less at the
...
clean exact checkout of the freeze branch head):
    ...
    python experiments/oracle/s6_m14_count.py --preflight \
        --freeze-rev <full 40-hex freeze branch head>
    python experiments/oracle/s6_m14_count.py \
        --freeze-rev <full 40-hex freeze branch head>
"""
...
import s6_m14_count_reservation as reservation  # noqa: E402
...
_MANIFEST = (_REPO / "docs" / "prereg"
             / "p14_s6_m14_count_freeze_manifest.json")
_ARTIFACT = _REPO / "docs" / "prereg" / "p14_s6_m14_count.json"
_INCIDENT = _REPO / "docs" / "prereg" / "p14_s6_m14_count_incident.json"
_CHECKPOINT = (_REPO / "docs" / "prereg"
               / "p14_s6_m14_count_checkpoint.json")
...
FROZEN = {
    ...
    "tau": 0.025,
    "alpha": 0.05,
    "v_lo": 64.33619774000562,
    "v_hi": 64.98274404119803,
    "v_ref": 64.65947089060182,
    "u_power_ceiling": 30,
    "seed_name": "s6_m14_count",
    "tol": s1.DEFAULT_TOL,
    "chunk": 65_536,
    "scan_call_cap": 34_000_000,
    "max_calls": 34_006_517,
    "max_wall_s": 86_400.0,
}
...
if any(_g[k] != v for k, v in _RUNG.items()):
    raise SystemExit(
        f"s6_m14_count: frozen rung literals drifted from the one "
...
def _import_gate() -> None:
    ...
    o3p = json.loads((_REPO / "docs" / "prereg"
                      / "p14_s6_m14_volume.json").read_text(
                          encoding="utf-8"))
    r = o3p["result"]
    if (r["v_lo"] != FROZEN["v_lo"] or r["v_hi"] != FROZEN["v_hi"]
            or r["status"] != "target-met"):
        raise SystemExit(
            f"s6_m14_count: the frozen oracle endpoints "
...
def main() -> None:
    ...
    try:
        ...
        result = {
            "kind": "results",
            "run_kind": "s6_m14_count",
            "stage": ("S6 M=1.4 Poisson-count campaign: sprinkled "
...
PROTOCOL_SURFACE = (
    "experiments/oracle/s6_m14_count.py",
    "experiments/oracle/s6_m14_count_reservation.py",
    ...
    "docs/prereg/p14_s6_m14_count.md",
    "docs/prereg/p14_s6_m14_volume.json",
    "docs/prereg/p14_s6_m14_pilot.json",
...
def build_manifest() -> dict:
    return {
        "stage": ("S6 M=1.4 Poisson-count campaign freeze manifest "
