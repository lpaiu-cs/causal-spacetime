"""O5 ambiguity pilot -- the frozen runner.

EXECUTION FOLLOWS THE DELEGATION RULING: after this freeze PR's review
converges and it merges, the pilot executes ONCE from the clean exact
checkout of the freeze branch head (the merge commit's second parent),
with no further per-step approval -- and NEVER twice. No rerun, no
seed reuse, no sample extension, no cap raise.

WHAT THE PILOT MEASURES. One fresh stream (`o5_amb_pilot`) draws
n = 10,736,965 events from the SAME 4D box measure the O5 campaign
will sprinkle (r^2 sin(psi) on r in [R_LO, R_HI], psi <= PSI_MAX,
phi in [0, 2pi), t in [0, DT]). For each event the full tri-state
`causal_relation` is asked about both legs (p -> x, then x -> q,
early exit); the event is AMBIGUOUS iff any asked leg returns None.
The ambiguous count k out of the fixed n is the whole observation.

THE FROZEN DECISION RULE IS GENERAL IN k -- nothing is hardcoded to
k <= 1:
    ...
    lambda_U = A_provisional * SCALE * p_upper   (Poisson thinning
               of the campaign point process);
    tail     = exact P(Poisson(lambda_U) > U_max), a finite sum;
    verdict  = FEASIBLE      if tail <= tail_budget
               INCONCLUSIVE  otherwise.

No normal approximation anywhere. The same functions that decide are
cross-checked by contract tests against the frozen P14 P2 engine and
an independent 96-bit gmpy2 engine.

FIXED n. The sample size never extends: a k larger than hoped is
published as INCONCLUSIVE exactly as computed. The verdict is about
the FEASIBILITY of the O5 power sentence at U_max, not about the
instrument -- an INCONCLUSIVE pilot indicts nothing and triggers a
redesign ruling, never an automatic retry.
...
freeze branch head):
    ...
    python experiments/oracle/o5_amb_pilot.py --preflight \
        --freeze-rev <full 40-hex freeze branch head>
    python experiments/oracle/o5_amb_pilot.py \
        --freeze-rev <full 40-hex freeze branch head>
"""
...
import o5_pilot_reservation as reservation  # noqa: E402
...
_MANIFEST = (_REPO / "docs" / "prereg"
             / "p14_o5_amb_pilot_freeze_manifest.json")
_ARTIFACT = _REPO / "docs" / "prereg" / "p14_o5_amb_pilot.json"
_INCIDENT = _REPO / "docs" / "prereg" / "p14_o5_amb_pilot_incident.json"
_CHECKPOINT = (_REPO / "docs" / "prereg"
               / "p14_o5_amb_pilot_checkpoint.json")
...
FROZEN = {
    ...
    "tol": s1.DEFAULT_TOL,
    "chunk": 65_536,
    "max_calls": 21_480_467,        # = 2*n_events + G3A_PREFLIGHT_CALLS
    "max_wall_s": 86_400.0,
}
assert FROZEN["max_calls"] == 2 * FROZEN["n_events"] + G3A_PREFLIGHT_CALLS

#: The anchors, exactly the campaign's.
...
def decide(k: int, cfg: dict = FROZEN) -> dict:
    ...
    lam_u = cfg["a_provisional"] * sz.SCALE * p_up
...
MANIFEST_NOTE = (
    "raw sha256; all paths .gitattributes eol=lf pinned. The pilot "
    "inherits the O4b instrument surface (predicate, wrapper "
    "preflight, budget/meter) and the campaign box measure from "
    "o4_sizing; the reservation machinery is o4b_reservation's, "
    "imported not copied. This manifest cannot certify itself: the "
    "run also demands the exact freeze branch head via --freeze-rev."
)
