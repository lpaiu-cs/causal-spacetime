"""Aggregate v4 protocol carry-forward evaluation into one report card."""
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V4 protocol report card.")
...
def run_experiment(config: ExperimentConfig) -> list[dict[str, float | str]]:
    ...
    registry = {
        ...
        for row in read_csv_rows(
            data_path(
                config.output_dir, "v4_protocol_carry_forward_registry_export.csv"
            )
        )
    }
...
def main() -> None:
    ...
    path = write_csv(
        ...
        data_path(config.output_dir, "v4_protocol_carry_forward_report_card.csv"),
        ["family_name", "decision", "interpretation_warning"],
    )
    print(f"Wrote v4 protocol carry-forward report card: {path}")
