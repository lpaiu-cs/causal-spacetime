"""Final sanity checks for M45 v4 protocol carry-forward evaluation."""
...
def run_experiment(output_dir: Path = Path("outputs")) -> list[dict[str, float | str]]:
    ...
    plan_rows = read_csv_rows(
        data_path(output_dir, "v4_protocol_stress_test_handoff_plan.csv")
    )
    ...
    decisions = read_csv_rows(
        data_path(output_dir, "v4_protocol_cross_family_robustness_decisions.csv")
    )
    docs_text = "\n".join(
        [
    ...
    checks = [
        (
            "v4_protocol_decisions_file_exists",
            data_path(
                output_dir, "v4_protocol_cross_family_robustness_decisions.csv"
            ).exists(),
        ),
        ("v4_protocol_registry_json_serializes", bool(registry.get("registry_id"))),
        ("blocked_report_only_failed_control_blocked_in_plan", blocked_rows_blocked),
        (
            "no_v4_protocol_stress_test_result_output",
            not any((output_dir / "data").glob("v4_protocol_*stress_test_result*.csv")),
        ),
        (
        ...
        (
            ...
            "Top-down/hybrid provenance is admissibility metadata, not evidence"
            in docs_text,
        ),
        (
...
def main() -> None:
    ...
    path = write_csv(
        ...
        data_path(output_dir, "v4_protocol_carry_forward_final_sanity.csv"),
        ["check", "passed"],
    )
    print(f"Wrote v4 protocol carry-forward final sanity: {path}")
