"""Drill down v3 protocol blocked results to manifest-level variance."""
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V3 manifest-level drilldown.")
...
def run_experiment(
    ...
    rows = load_v3_protocol_manifest_drilldown_rows(config.output_dir)
    return (
        [manifest_drilldown_row_to_csv(row) for row in rows],
        summarize_manifest_drilldown_by_family(rows),
    )
...
def main() -> None:
    ...
    paths = [
        write_csv(
            rows,
            data_path(config.output_dir, "v3_protocol_manifest_level_drilldown.csv"),
            ["manifest_id", "family_name", "heldout_violation_rate"],
        ),
        write_csv(
            summary,
            data_path(config.output_dir, "v3_protocol_manifest_level_summary.csv"),
    ...
    figure = save_metric_bar(
        ...
        / "figures"
        / "v3_protocol_manifest_heldout_variance.png",
        ylabel="held-out std",
    )
    print("Wrote v3 protocol manifest drilldown: " + ", ".join(map(str, paths)))
