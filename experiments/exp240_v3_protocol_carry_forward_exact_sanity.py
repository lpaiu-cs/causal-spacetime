"""Exact sanity checks for v3 protocol carry-forward decisions."""
...
def _precondition(name: str, passed: bool = True) -> V3ProtocolPreconditionReport:
...
def run_experiment() -> list[dict[str, float | str]]:
    """Run deterministic decision sanity rows."""
...
def main() -> None:
    path = write_csv(
        run_experiment(),
        Path("outputs/data/v3_protocol_carry_forward_exact_sanity.csv"),
        ["check", "passed"],
    )
    print(f"Wrote v3 protocol carry-forward exact sanity: {path}")
