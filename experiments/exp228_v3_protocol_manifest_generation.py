...
from causal_spacetime_lab.state_change_manifest_v3_protocol_execution_spec import (
    ...
    v3_protocol_execution_spec_table,
)
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="Generate patched v3 manifests.")
...
def run_experiment(config: ExperimentConfig) -> list[dict[str, float | str]]:
    ...
    specs = load_v3_protocol_execution_specs(spec_path)
    if not specs:
        return [
            {
    ...
    executed_specs = mark_v3_protocol_execution_specs_executed(specs)
    profile_configs = default_v3_protocol_profile_configs(specs, seed=config.seed)
    ...
    rows = v3_protocol_execution_spec_table(executed_specs)
    for row in rows:
...
def main() -> None:
    ...
    path = write_csv(
        rows,
        data_path(config.output_dir, "v3_protocol_manifest_generation.csv"),
        ["status", "missing_input", "manifest_count"],
    )
    print(f"Wrote v3 protocol manifest generation: {path}")
