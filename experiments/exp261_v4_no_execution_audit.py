...
from v3_protocol_blocked_v4_experiment_helpers import data_path, read_json
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V4 no-execution audit.")
        ...
        return [
            {
                ...
                "passed": 0.0,
                "detail": "missing_v4_preregistration_spec",
            }
        ]
    spec = build_v4_protocol_preregistration_spec(
        default_v4_protocol_family_designs()
    )
    audit = audit_v4_preregistration_only(spec)
    no_outputs = check_no_v4_execution_outputs(config.output_dir)
    rows = [
        {"check": key, "passed": float(value), "detail": ""}
        for key, value in {**audit, **no_outputs}.items()
    ]
    rows.insert(0, {"check": "v4_preregistration_exists", "passed": 1.0, "detail": ""})
    return rows
...
def main() -> None:
    ...
    path = write_csv(
        ...
        data_path(config.output_dir, "v4_no_execution_audit.csv"),
        ["check", "passed", "detail"],
    )
    print(f"Wrote v4 no-execution audit: {path}")
