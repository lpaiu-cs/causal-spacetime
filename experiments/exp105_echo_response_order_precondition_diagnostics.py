"""Precondition diagnostics for consensus echo-response order signatures."""
...
from causal_spacetime_lab.state_change_response_signature import signature_tie_fraction
...
def parse_args() -> ExperimentConfig:
    ...
    parser = argparse.ArgumentParser(
        description="Echo response order precondition diagnostics."
    )
...
def _nonzero_pair_fraction(matrix: np.ndarray) -> float:
...
def run_experiment(config: ExperimentConfig) -> list[dict[str, float]]:
    """Run consensus response-order precondition diagnostics."""
        ...
        rows.append(
            {
                ...
                "consensus_nonzero_pair_fraction": _nonzero_pair_fraction(consensus),
                "cycle_count": float(response_order_cycle_count(consensus)),
                "mean_tie_fraction": float(
                    np.mean([signature_tie_fraction(sig) for sig in signatures])
                ),
            }
        )
    return rows
...
def write_outputs(rows: list[dict[str, float]], output_dir: Path) -> Path:
    ...
    path = output_dir / "data" / "echo_response_order_precondition_diagnostics.csv"
...
def save_figures(rows: list[dict[str, float]], output_dir: Path) -> list[Path]:
    ...
    paths: list[Path] = []
    x_values = np.asarray([row["repetition"] for row in rows], dtype=float)
    for value, filename, ylabel in [
        (
            "cycle_count",
            "echo_response_consensus_cycle_count.png",
            "directed 3-cycle count",
        ),
        (
            "stable_pair_fraction",
            "echo_response_consensus_stable_pair_fraction.png",
            "stable pair fraction",
        ),
    ]:
        y_values = np.asarray([row[value] for row in rows], dtype=float)
...
def main() -> None:
    ...
    print(f"Wrote echo response order precondition diagnostics: {data_path}")
