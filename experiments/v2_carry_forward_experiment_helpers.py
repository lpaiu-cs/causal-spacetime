"""Shared helpers for Milestone 38 v2 carry-forward experiments."""
...
def read_v2_registry(output_dir: Path) -> CarryForwardRegistry | None:
    """Read the v2 carry-forward registry if present."""

    path = output_dir / "carry_forward_v2" / "carry_forward_registry_v2.json"
    if not path.exists():
        return None
    payload = json.loads(path.read_text(encoding="utf-8"))
    return registry_from_jsonable(payload)
