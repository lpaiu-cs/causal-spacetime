"""Null taxonomy diagnostics for protocol-invariant v3 manifests."""
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V3 protocol null taxonomy.")
    parser.add_argument(
...
def main() -> None:
    config = parse_args()
    rows = compute_v3_protocol_null_taxonomy_rows(
        config.manifest_dir,
    ...
    path = write_csv(
        ...
        ["manifest_id", "family_name", "null_type", "taxonomy_class"],
    )
    save_family_metric_figure(
        rows,
        metric="mean_heldout_violation_rate",
        path=Path(
            "outputs/figures/v3_protocol_manifest_null_taxonomy_heldout_violation.png"
        ),
        ylabel="Mean null heldout violation",
    )
    print(f"Wrote v3 protocol null taxonomy diagnostics: {path}")
