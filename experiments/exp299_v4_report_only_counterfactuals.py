"""Run report-only v4 counterfactual diagnostics."""
...
from causal_spacetime_lab.state_change_manifest_v4_counterfactuals import (
    destructive_null_gap_counterfactual_report,
...
class ExperimentConfig:
    ...
    destructive_null_thresholds: tuple[float, ...] = (0.05, 0.10, 0.15)
...
def parse_args() -> ExperimentConfig:
    parser = argparse.ArgumentParser(description="V4 report-only counterfactuals.")
    ...
    return ExperimentConfig(
        ...
        destructive_null_thresholds=_parse_values(args.destructive_null_thresholds),
...
def run_experiment(
    ...
    precondition_rows = to_float_rows(
        read_csv_rows(
            data_path(config.output_dir, "v4_protocol_precondition_audit.csv")
        )
    )
    ...
    return (
        ...
        destructive_null_gap_counterfactual_report(
            metric_rows,
            hypothetical_thresholds=list(config.destructive_null_thresholds),
        ),
        single_fix_rows,
    )
...
def main() -> None:
    config = parse_args()
    heldout, latent, destructive, single_fix = run_experiment(config)
    paths = [
        write_csv(
            heldout,
            data_path(config.output_dir, "v4_heldout_counterfactual.csv"),
            ["family_name", "hypothetical_threshold", "note"],
        ),
        write_csv(
            latent,
            data_path(config.output_dir, "v4_latent_order_counterfactual.csv"),
            ["family_name", "hypothetical_threshold", "note"],
        ),
        write_csv(
            destructive,
            data_path(config.output_dir, "v4_destructive_null_counterfactual.csv"),
            ["family_name", "hypothetical_threshold", "note"],
        ),
        write_csv(
            single_fix,
            data_path(config.output_dir, "v4_single_fix_counterfactual.csv"),
            ["family_name", "ignored_root_cause", "note"],
        ),
    ]
    print("Wrote v4 report-only counterfactuals: " + ", ".join(map(str, paths)))
