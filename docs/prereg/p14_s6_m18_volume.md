# S6 rung M = 1.8 — oracle freeze (certified volume at target 0.005)

Freeze document (2026-08-17, S6 ruling). **No results. Execution is
NOT approved by this freeze**: the run starts only after the M = 1.8
...
The mass ladder's deepest rung. The certified shell `[10, 20]`, the
...
| --- | --- | --- |
| `m` | **1.8** | S6 ruling |
| `mu = 2M/r_c` (r_c = 15) | **0.24000000000000002** | the pre-frozen indicator (central rung: 0.1333…) |
| `T_min(1.8)` | 7.940387402637673 | `rho(18) − rho(12)` at m = 1.8 |
| `dt` | **9.725248174609407** | the ladder convention `8.5 · T_min(M)/T_min(1)`, one exact path (`s6_rungs`) |
| box `r` | [11.38246192411518, 18.717403954190363] | containment certificate, outward binary64 |
| `psi_max` | 0.519895801069135 | `dt / (2 w_glob)` |
| `SCALE` | 13679.093767152488 | `dt · B_box`, the o4_sizing exact path |
...
+1.78/+4.17/+3.15/+8.98 for L4, +33.35 for winding). Nothing is
...
- `v_ref_rung` as the standalone midpoint, **recommendation only**:
  ...
  particular the survey's A = 710 and pilot n ≈ 3.9M are NOT frozen
  anywhere.
