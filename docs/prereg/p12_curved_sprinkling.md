...
(P11 9.6's lesson: pilots publish the calibrated bound they feed to
power). Stage P-12 pilots BOTH endpoint rungs, 200 samples each,
...
- **Frozen patch constants**: `ell = 1`, `eta in [-2.0, -1.0]`,
  `|x| <= 0.5`. Proper volume of the patch is finite and computed
...
- **Density calibration**: as in P11, the instrument is given the
  count (hence `rho`, via the patch volume it is also given). The
...
- Pairs: `K = 6` per sample from the precomputed pool of related
  pairs whose `tau_curved` lies in the frozen band
  `[0.28, 0.34]` (so `tau / ell ~ 0.3`), drawn without replacement
  with the P11 greedy disjoint-support rule (supports are the pairs'
  `(U, V)` boxes, exactly as in P11 Stage A), 200-draw cap on
  overlap rejections, first-`n`-complete fill, skip cap 20.
...
truth of Section 3, the pair pool and protocol, and stage runners
verify/pilot/A. Estimators come from the shared definitions used by
