...
`hi = Δt − T₂`, `L = hi − lo` 로 두면 (아래 값들은 **실수 산술에서의**
항등식이다 — 부동소수점에서의 단서는 §5a):
...
### 5a. 부동소수점에서는 "정확히" 가 아니다

위 표의 `1e-6` 은 **실수 산술에서의 값**이다. 실제로 술어가 판정에 쓰는
간격은 `dt = Δt − t_x` 와 `\|dt − t_min\|` 를 부동소수점으로 형성한
결과이므로 `1e-6` 과 **반올림 규모까지만** 같다. 관측된 첫 undecided 에서
그 값은 `9.99999999695...e-07` 이었다.

따라서 두 조건은 원리적으로 갈릴 수 있다:

| 이름 | 조건 | 뜻 |
|---|---|---|
| nominal | `err₂ ≥ 1e-6` | 동결 offset 과의 비교 |
| realized | `err₂ ≥ \|dt − t_min\|` | 술어가 **실제로** 한 비교 |

`err₂` 가 `1e-6` 의 반올림 근방에 떨어지는 클러스터에서만 갈리므로 드물
것이나, **드문 것과 없는 것은 다르다.** census 는 둘을 각각 세고 갈리는
개수와 **첫 좌표**까지 기록하며, 같다고 단언하지 않는다(§6a). 이 갈림
자체가 §5 결함군의 또 다른 표식이다 — 고정된 값 하나를 여러 곳에서
"정확히" 라고 부르는 데서 온다.

계약 테스트도 이 의미론을 따른다: 간격이 offset 과 **`1e-12` 이내로**
같음을 요구할 뿐 비트 동일을 요구하지 않는다.
...
는 측정할 뿐이다. 간격이 `1e-6` 과 **정확히** 같지는 않다는 단서는
§5a 에 있다.
