...
**근거 정정 (철회).** 초안은 "한 걸음이 최소 `ulp(1.0)` 를 벌므로 약 17
걸음이면 충분하고 64 는 3.7 배 여유" 라고 적었다. **이 논증은
철회한다.** 걸음당 이득은 `ulp(1.0)` 도 아니고 `ulp(dt)` 도 아니다 —
여유는 `|dt − t_min| − err` 이고 그 차가 **`err` 규모에서 형성**되므로,
`dt` 를 1 ulp 움직여도 여유는 그보다 훨씬 적게 움직인다. G3a 표의 한
사례(`t_min = 0.0026643057799644846`, `err = 0.0026640756792607376`,
`dt = 2.3×10⁻⁷`)는 **8,042 걸음**을 요구한다 — 추정이 아니라 루프를
실제로 돌려 센 값이다. **유한한 예산으로 모든 부족분이 닫힌다는 보장은
없다.**

정확한 진술은 이것이다:

> `MAX_NUDGES = 64` 는 모든 부족분을 해소한다는 수학적 보장이 아니라,
> 1-ulp outward search 의 **사전 동결 계산 예산**이다. 그 안에서 실현
> 여유를 확보하지 못하면 **`construction-unavailable`** 로 기록하며,
> mismatch 또는 instrument failure 로 재분류하지 않는다.

예산은 **이동 횟수**를 센다: 최초 배치를 먼저 검사하고, 그 뒤 최대
`MAX_NUDGES` 회 바깥으로 움직인다. 마지막 검사가 실패해도 더 움직이지
않는다.
