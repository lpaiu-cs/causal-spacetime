...
초안은 `MAX_NUDGES = 64` 를 동결하고 "한 걸음이 최소 `ulp(1.0)` 를
벌므로 약 17 걸음이면 충분, 64 는 3.7 배 여유" 로 정당화했다. **그 논증은
철회한다** — 여유는 `|dt − t_min| − err` 이고 그 차가 **`err` 규모에서
형성**되므로, `dt` 를 1 ulp 움직여도 여유는 그보다 적게 움직인다. 표의 한
사례는 **8,042 걸음**을 요구한다(루프를 돌려 센 값).

**그 다음이 더 중요한 정정이다.** 첫 대응은 64 를 유지하고 그 사례를
`construction-unavailable` 로 기록하는 것이었다. **그것은 틀렸다.**
8,042 는 "그 사례를 버려야 한다" 는 증거가 아니라 **"임의 cap 이 결함"**
이라는 증거다. 완벽히 구성 가능한 probe 를 숫자 하나 때문에 버리는 것은,
실패 전체가 **적응하지 않는 여유** 였던 stage 에 같은 것을 하나 더
들이는 일이다.

그래서 탐색은 **실현 여유에 도달할 때까지 계속한다.** 종료는 예산이
아니라 **표현 가능성**이 정한다:

| 종료 조건 | 처분 |
|---|---|
| `dt` 가 비유한 | `construction-unavailable` |
| `nextafter` 가 더 움직이지 못함 | `construction-unavailable` |
| 하단 probe 에서 `dt < 0` | `construction-unavailable` (단락 영역, 행 D 의 몫) |
| 실현 여유 `≥ η` | **성공** |

종료는 보장된다 — 위쪽으로는 여유가 단조 증가하고, 아래쪽으로는 `dt` 가
감소해 0 경계를 만난다.

**거리는 진단값이며 비용이 아니다.** `ulp_distance` 는 명목 배치가
만족하는 배치에서 몇 칸 떨어져 있었는지이고, 실제 수행량은
`search_comparisons` 로 따로 기록한다. 둘을 한 이름으로 두면 거리가
실행 횟수로 오독된다.

### 4.3a 탐색 방식 — 단조 방향에만 지수 탐색

여유가 한 방향으로 단조 증가하는 probe 는 float **순서** 위에서 지수
bracket 후 이분한다. 8,042 칸 떨어진 사례도 27 회 비교로 **같은 최초
만족값**(비트 동일)을 찾는다. G3b 는 100,000 클러스터에 probe 를 놓으므로
1 ulp 씩 걷는 구현은 새로운 정체 위험이다.

**그러나 inside probe 에는 쓸 수 없다.** 한 leg 의 여유를 넓히면 다른
leg 의 여유가 줄어들어 **성공영역이 반직선이 아니라 구간**이고, 지수
점프가 그 구간을 통째로 건너뛸 수 있다.

| probe | 여유의 모양 | 탐색 |
|---|---|---|
| G3a 행 A · 행 B | 한 방향 단조 | 지수 bracket + 이분 |
| G3b outside-above · outside-below | 한 방향 단조 | 지수 bracket + 이분 |
| **G3b inside** | **구간** | **두 leg 의 표현 가능 범위 교집합을 직접 구하고, 선택값에서 양쪽 조건을 재검증** |

inside probe 는 위 탐색 루틴을 **호출해서는 안 된다.** 이 제약은 코드
쪽에도 명시돼 있다.
...
**η 는 여기 없다.** census 없이 §4 에서 정해졌고 census 가 바꾸지
않는다. 사후 동결 대상이 아님을 명시해 두는 이유는, 그렇지 않으면 census
를 본 뒤에 고를 여지가 남기 때문이다.

**탐색 cap 도 여기 없다 — 아예 존재하지 않기 때문이다**(§4.3).
