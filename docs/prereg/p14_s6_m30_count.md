# S6 M=3.0 Poisson-count campaign — freeze (sprinkled tri-state membership count)
...
rung's certified interval (M = 3.0, μ = 0.4). The **estimand is `V_op − V_true`**, where
...
| key | value |
| --- | --- |
| `a_intensity` | **440.0** per unit 4-volume (final; re-frozen from the actual rung endpoints) |
| `tau` | **0.025** (equivalence band `B = tau · V_ref = 3.045853763319425`) |
| `alpha` | 0.05 (exact Garwood, alpha/2 per side, ONE outer interval) |
| `v_lo`, `v_hi` | **121.22502058308072, 122.44328048247328** (the rung artifact verbatim, target-met at ratio 0.004999) |
| `v_ref` | **121.834150532777** (the rung artifact's recommendation, adopted HERE as deferred by PR #108) |
| `u_power_ceiling` | 30 (certified by the rung pilot, FEASIBLE at k = 0) |
| seed | `s6_m30_count = 40,000,511` (FRESH; spent by the run) |
| `tol` | 1e-08 (the campaign predicate's) |
| `chunk` | 65,536 |
| `scan_call_cap` | **11,000,000** |
| `max_calls` | **11,006,466 = scan cap + 6,466** (the rung-measured G3a preflight at m = 3.0 and the rung geometry, charged to the same budget) |
| `max_wall_s` | 86,400 |

Caps are frozen — **no auto-raise, ever**. The wall cap is a stop
rule, not part of any power statement. `E[calls] ≤ 2·A·SCALE_rung =
9,215,381` (~2.3 h hard; less at the observed early-exit rate —
far under the wall).

The frozen endpoints are not merely asserted: the module refuses to
import if `p14_s6_m30_volume.json` does not carry exactly these
serialized endpoints with `target-met`, or if `p14_s6_m30_pilot.json`
...
```
acceptance on K_true (U = 0)     [52,986, 54,220]
   (equivalently: decide(k, 0) = CONCORDANT exactly for k in that window)
endpoint powers                   0.937123 @ V_lo   0.931493 @ V_hi
worst power                       0.931493  (≥ the 0.93 proposal margin)
power at U = 30 (window shrink)   0.912741
joint completion-and-concordance  ≥ 0.912741 − 0.001 (pilot tail budget)
                                    − 0.010 (pilot alpha)
                                    − exp(−81,307) (call-cap Chernoff)
                                  = 0.901741 ≥ 0.90
```

The Chernoff term is an exact INEQUALITY upper bound on
`P(2N > 11,000,000)` under `N ~ Poisson(A·SCALE_rung)`, not the exact
...
`refs/s6m30pilot`, namespace probe on `refs/s6m30/`) →
metered G3a full-wrapper preflight (6,466 deterministic calls,
...
- **Execution gate.** This freeze does NOT authorize execution. The
  ...
  fresh seed, retained refs and an unclaimed `refs/s6m30/reservation`.
- **Lineage.** The campaign stream (`40,000,511`) is fresh and
