# S6 M=1.8 Poisson-count campaign — freeze (sprinkled tri-state membership count)
...
rung's certified interval (M = 1.8, μ = 0.24000000000000002). The **estimand is `V_op − V_true`**, where
...
| key | value |
| --- | --- |
| `a_intensity` | **720.0** per unit 4-volume (final; re-frozen from the actual rung endpoints) |
| `tau` | **0.025** (equivalence band `B = tau · V_ref = 1.858506959191902`) |
| `alpha` | 0.05 (exact Garwood, alpha/2 per side, ONE outer interval) |
| `v_lo`, `v_hi` | **73.96863670309186, 74.7119200322603** (the rung artifact verbatim, target-met at ratio 0.004999) |
| `v_ref` | **74.34027836767608** (the rung artifact's recommendation, adopted HERE as deferred by PR #98) |
| `u_power_ceiling` | 30 (certified by the rung pilot, FEASIBLE at k = 0) |
| seed | `s6_m18_count = 40,000,491` (FRESH; spent by the run) |
| `tol` | 1e-08 (the campaign predicate's) |
| `chunk` | 65,536 |
| `scan_call_cap` | **23,000,000** |
| `max_calls` | **23,006,461 = scan cap + 6,461** (the rung-measured G3a preflight at m = 1.8 and the rung geometry, charged to the same budget) |
| `max_wall_s` | 86,400 |

Caps are frozen — **no auto-raise, ever**. The wall cap is a stop
rule, not part of any power statement. `E[calls] ≤ 2·A·SCALE_rung =
19,697,895` (~5.0 h hard; less at the observed early-exit rate —
far under the wall).

The frozen endpoints are not merely asserted: the module refuses to
import if `p14_s6_m18_volume.json` does not carry exactly these
serialized endpoints with `target-met`, or if `p14_s6_m18_pilot.json`
...
```
acceptance on K_true (U = 0)     [52,905, 54,137]
   (equivalently: decide(k, 0) = CONCORDANT exactly for k in that window)
endpoint powers                   0.936950 @ V_lo   0.931336 @ V_hi
worst power                       0.931336  (≥ the 0.93 proposal margin)
power at U = 30 (window shrink)   0.912536
joint completion-and-concordance  ≥ 0.912536 − 0.001 (pilot tail budget)
                                    − 0.010 (pilot alpha)
                                    − exp(−131,245) (call-cap Chernoff)
                                  = 0.901536 ≥ 0.90
```

The Chernoff term is an exact INEQUALITY upper bound on
`P(2N > 23,000,000)` under `N ~ Poisson(A·SCALE_rung)`, not the exact
...
`refs/s6m14pilot`, `refs/s6m14` AND
`refs/s6m18pilot`, namespace probe on `refs/s6m18/`) →
metered G3a full-wrapper preflight (6,461 deterministic calls,
...
- **Execution gate.** This freeze does NOT authorize execution. The
  ...
  fresh seed, retained refs and an unclaimed `refs/s6m18/reservation`.
- **Lineage.** The campaign stream (`40,000,491`) is fresh and
