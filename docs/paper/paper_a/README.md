...
- `manuscript.md` — the draft (all sections).
- `claim_boundary.md` — claim/non-claim checklist.
- `artifact_manifest.md` — the archived-state authority: SHA-256 digests and
  row counts for all 19 committed legacy tables, the Section 6 capstone and
  6.7 extension bundles, and the auxiliary O4b instrument-audit bundle, each
  recomputed by a contract test.
...
19-table evidence package is committed and digest-locked, regenerated once
in a recorded environment from the unmodified baseline producers;
`artifact_manifest.md` records the regeneration and every digest, and a
contract test recomputes them, so the draft is submission-reproducible from
a clean checkout.
