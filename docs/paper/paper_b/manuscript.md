...
lambda ~ rho x ell. Three density-coupled protocols now exist as
...
events nearest the clock-window endpoints — a coordinate-assisted
designation made once at setup, like placing observers), the harvest is
a longest causal chain between the anchors: the selection rule reads
order data alone, and an order-only causal set with the anchors
labelled reproduces it exactly. Its expectations were stated before the
grid ran, directionally for the fluctuation mechanism: freed from the
...
classification is descriptive characterization on the theory track,
