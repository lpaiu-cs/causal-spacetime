"""V3 protocol future stress-test handoff utilities."""
...
def evaluate_v3_protocol_stress_test_stop_condition(
    decisions: list[FamilyRobustnessDecision],
) -> StopConditionReport:
    """Evaluate the v3 protocol future stress-test stop condition."""
...
def build_v3_protocol_stress_test_plan(
    registry: CarryForwardRegistry,
) -> list[StressTestPlanEntry]:
    """Build a future stress-test handoff plan from the v3 protocol registry."""

    return build_stress_test_plan(registry)


def v3_protocol_stress_test_plan_rows(
    entries: list[StressTestPlanEntry],
) -> list[dict[str, float | str]]:
    """Convert v3 protocol stress-test handoff entries to CSV rows."""
