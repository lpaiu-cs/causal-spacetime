"""Manifest-level drilldown utilities for blocked v4 protocol decisions."""
...
class V4ManifestDrilldownRow:
    """Manifest-level diagnostic row for v4 blocked-decision audits."""
...
def load_v4_manifest_drilldown_rows(output_dir: Path) -> list[V4ManifestDrilldownRow]:
    """Load manifest-level diagnostic rows from M44 v4 protocol outputs."""

    data_dir = output_dir / "data"
    fit_rows = _read_csv(data_dir / "v4_protocol_manifest_family_fit_comparison.csv")
    coverage_rows = _read_csv(
        data_dir / "v4_protocol_manifest_family_coverage_metrics.csv"
    )
    restart_rows = _read_csv(
        data_dir / "v4_protocol_manifest_family_restart_stability.csv"
    )
    latent_rows = _read_csv(
        data_dir / "v4_protocol_manifest_family_latent_order_stability.csv"
    )
    metadata_rows = _read_csv(data_dir / "v4_protocol_manifest_metadata_audit.csv")
    failed_rows = _read_csv(
        data_dir / "v4_protocol_manifest_family_failed_accounting.csv"
    )
    ...
    rows: list[V4ManifestDrilldownRow] = []
...
def summarize_v4_manifest_drilldown_by_family(
    rows: list[V4ManifestDrilldownRow],
) -> list[dict[str, float | str]]:
    """Summarize manifest-level variance by family."""

    grouped: dict[str, list[V4ManifestDrilldownRow]] = defaultdict(list)
    for row in rows:
