"""Criterion-margin utilities for v4 blocked-decision audits."""
...
def _margin(record: V4ProtocolBlockingRecord) -> float:
...
def criterion_margins_from_v4_blocking_records(
    records: list[V4ProtocolBlockingRecord],
) -> list[V4CriterionMargin]:
    """Convert v4 blocking records to signed criterion margins."""

    margins: list[V4CriterionMargin] = []
...
def criterion_margin_to_row(margin: V4CriterionMargin) -> dict[str, float | str]:
...
def summarize_v4_criterion_margins(
    margins: list[V4CriterionMargin],
) -> list[dict[str, float | str]]:
    """Summarize worst criterion margin and failure counts by family."""

    grouped: dict[str, list[V4CriterionMargin]] = defaultdict(list)
