"""V3 protocol carry-forward registry utilities."""
...
def _v3_protocol_assignments_from_manifest_dir(
    manifest_dir: Path,
...
def build_v3_protocol_carry_forward_registry(
    decisions: list[FamilyRobustnessDecision],
    manifest_dir: Path,
    criteria_name: str = "fixed_m34_v3_protocol",
) -> CarryForwardRegistry:
    """Build a v3 protocol carry-forward registry from decisions."""

    registry = build_carry_forward_registry(
        decisions,
        manifest_assignments=_v3_protocol_assignments_from_manifest_dir(manifest_dir),
        criteria_name=criteria_name,
    )
    registry.created_by_milestone = "Milestone 42"
    return registry


def write_v3_protocol_carry_forward_registry(
    registry: CarryForwardRegistry,
    output_dir: Path,
) -> Path:
    """Write the v3 protocol carry-forward registry JSON."""
...
def v3_protocol_registry_summary_rows(
    registry: CarryForwardRegistry,
) -> list[dict[str, float | str]]:
    """Return CSV-safe summary rows for a v3 protocol registry."""
