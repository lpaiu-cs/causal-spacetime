...
def write_v3_protocol_bundle(output_dir: Path, family_name: str) -> None:
    data_dir = output_dir / "data"
    metrics = [strong_metric_row(family_name)]
    write_csv(
        data_dir / "v3_protocol_cross_family_robustness_metrics.csv",
        metrics,
    )
    write_csv(
        data_dir / "v3_protocol_diagnostic_completeness_check.csv",
        [
            {
                "family_name": family_name,
    ...
    write_csv(
        data_dir / "v3_protocol_manifest_generation.csv",
        [
            {
                "family_name": family_name,
                "family_kind": "structured",
    ...
    for _key, filename in V3_PROTOCOL_OUTPUT_FILES.items():
