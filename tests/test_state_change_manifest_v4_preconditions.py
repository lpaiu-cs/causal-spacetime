...
from tests.v4_protocol_test_helpers import (
    family_from_manifest_dir,
    write_v4_protocol_bundle,
)


def test_evaluate_v4_protocol_preconditions_passes_valid_structured_manifest(
    tmp_path: Path,
    m44_manifest_dir: Path,
) -> None:
    family = family_from_manifest_dir(m44_manifest_dir)
    write_v4_protocol_bundle(tmp_path, family)
    bundle = load_v4_protocol_output_bundle(tmp_path)

    reports = evaluate_v4_protocol_preconditions(m44_manifest_dir, bundle)

    assert reports[0].preconditions_passed


def test_evaluate_v4_protocol_preconditions_blocks_missing_provenance(
    tmp_path: Path,
    m44_manifest_dir: Path,
) -> None:
    family = family_from_manifest_dir(m44_manifest_dir)
    ...
    broken_dir.mkdir()
    payload = json.loads(sorted(m44_manifest_dir.glob("*.json"))[0].read_text())
    payload.pop("handoff_provenance", None)
    (broken_dir / "broken.json").write_text(json.dumps(payload), encoding="utf-8")
    write_v4_protocol_bundle(tmp_path, family)

    reports = evaluate_v4_protocol_preconditions(
        broken_dir,
        load_v4_protocol_output_bundle(tmp_path),
    )

    assert "missing_handoff_provenance" in reports[0].failed_preconditions


def test_evaluate_v4_protocol_preconditions_blocks_protocol_mixed(
    tmp_path: Path,
    m44_manifest_dir: Path,
) -> None:
    family = family_from_manifest_dir(m44_manifest_dir)
