"""S6 rung M = 1.8 oracle-result contract tests.

The rung's certified volume was executed ONCE, from the clean exact
checkout of the freeze BRANCH HEAD `9f9a7e3` (the second parent of
merge `bac6886`, PR #93), under the frozen caps. These tests pin the
...
_FREEZE = "9f9a7e3577670772cefd37358cd8e8e78e07905a"
_MERGE = "bac6886"
_ART = _REPO / "docs" / "prereg" / "p14_s6_m18_volume.json"
_SNAP = (_REPO / "docs" / "prereg"
         / "p14_s6_m18_executed_freeze_manifest.json")

sys.path.insert(0, str(_REPO / "experiments" / "oracle"))

import s6_m18_frozen_volume as run  # noqa: E402
...
def test_termination_is_target_met_with_full_provenance():
    ...
    assert r["max_depth_reached"] == 13
    assert r["cells_at_max_depth"] == 0


def test_the_certified_interval_is_the_published_one():
    r = _artifact()["result"]
    assert r["v_lo"] == 73.96863670309186
    assert r["v_hi"] == 74.7119200322603
    assert r["calls"] == 502_950
...
def test_the_rung_identity_is_the_ladder_constant():
    art = _artifact()
    assert art["rung"]["m"] == 1.8
    assert art["rung"]["mu"] == s6.RUNG_CONSTANTS[1.8]["mu"]
    assert art["rung"]["scale"] == s6.RUNG_CONSTANTS[1.8]["scale"]
    assert art["frozen_config"]["dt"] == s6.RUNG_CONSTANTS[1.8]["dt"]
...
def test_executed_freeze_snapshot_matches_the_historical_blobs():
    hist = _blob("docs/prereg/p14_s6_m18_freeze_manifest.json")
