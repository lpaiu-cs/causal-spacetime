"""S6 rung M = 1.8 oracle-freeze contract tests.
...
discipline. NO RESULTS: execution waits on the M = 1.8 integrated
approval."""
...
def test_the_frozen_configuration_is_the_ladder_rung():
    ...
    assert run.FROZEN["dt"] == s6.RUNG_CONSTANTS[1.8]["dt"]
    assert run.FROZEN["mu"] == s6.RUNG_CONSTANTS[1.8]["mu"]
    assert run.FROZEN["dt"] == s6.dt(1.8)
...
def test_the_doc_freezes_the_discipline():
    doc = (_REPO / "docs" / "prereg"
           / "p14_s6_m18_volume.md").read_text(encoding="utf-8")
    flat = " ".join(doc.split())
    assert "Execution is NOT approved by this freeze" in flat
    assert "no auto-raise, ever" in flat
    assert "published as is" in flat
    assert "9.725248174609407" in flat
    assert "0.24000000000000002" in flat
    assert "recommendation only" in flat
    assert "A = 710 and pilot n ≈ 3.9M are NOT frozen" in flat
