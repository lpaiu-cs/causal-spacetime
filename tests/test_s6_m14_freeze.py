"""S6 rung M = 1.4 oracle-freeze contract tests.
...
discipline. NO RESULTS: execution waits on the M = 1.4 integrated
approval."""
...
def test_the_frozen_configuration_is_the_ladder_rung():
    ...
    assert run.FROZEN["dt"] == s6.RUNG_CONSTANTS[1.4]["dt"]
    assert run.FROZEN["mu"] == s6.RUNG_CONSTANTS[1.4]["mu"]
    assert run.FROZEN["dt"] == s6.dt(1.4)
...
def test_the_import_gate_refuses_a_drifted_constant(monkeypatch):
    monkeypatch.setitem(run.FROZEN, "dt", 9.07)
...
def test_the_doc_freezes_the_discipline():
    doc = (_REPO / "docs" / "prereg"
           / "p14_s6_m14_volume.md").read_text(encoding="utf-8")
    flat = " ".join(doc.split())
    assert "Execution is NOT approved by this freeze" in flat
    assert "no auto-raise, ever" in flat
    assert "published as is" in flat
    assert "9.070565190742672" in flat
    assert "0.18666666666666665" in flat
    assert "recommendation only" in flat
    assert "A = 820 and pilot n ≈ 5.9M are NOT frozen" in flat
