"""O5 Poisson-count campaign freeze contract tests.
...
def test_the_frozen_endpoints_are_the_o3p_artifact_verbatim():
    ...
    assert (o3p["v_ref_prime_recommendation"]["value"]
            == camp.FROZEN["v_ref"])
...
def test_the_seed_is_retired_and_301_untouched():
    """The results commit moved `o5_campaign` to OBSERVED in the same
    change that added the observed CONCORDANT artifact; the retired
    stream can never be allocated again, only replayed."""
    ...
    assert 40_000_451 in ledger.spent_scalars()
    with pytest.raises(KeyError, match="no fresh allocation"):
        ledger.assert_fresh_scalar("o5_campaign")
...
def test_the_freeze_commit_carried_the_campaign_scalar_as_fresh():
    """A historical assertion (the O4/pilot pattern): the executed
    freeze `c6eb85e` is what must have held the stream as FRESH."""

    blob = subprocess.run(
        ["git", "show",
         "c6eb85eea1affd33a1246627ea075dca96a859bb:experiments/"
    ...
    assert '"o5_campaign": 40_000_451' in frozen
...
def test_the_three_way_verdict_is_exhaustive_and_disjoint():
    cases = {
        ...
        (0, 0): "DISCORDANT",
        (52_000, 0): "INCONCLUSIVE",   # overlaps without containment
        (52_751, 0): "INCONCLUSIVE",
    }
...
def test_decide_publishes_its_whole_arithmetic():
    d = camp.decide(53_000, 5)
    assert d["k_certain"] == 53_000 and d["u_ambiguous"] == 5
    ...
    assert math.isclose(d["c_lo_volume"],
                        d["garwood_low_counts"] / 940.0,
                        rel_tol=1e-15)
    assert math.isclose(d["c_hi_volume"],
                        d["garwood_up_counts"] / 940.0,
                        rel_tol=1e-15)
    assert math.isclose(d["band"], 0.025 * 56.776812866643795,
                        rel_tol=1e-15)
...
def test_the_power_sentence_numbers_reproduce():
    ...
    assert abs(p_lo - 0.936665) < 5e-7
    assert abs(p_hi - 0.930794) < 5e-7
    assert p_hi < p_lo                      # the worst endpoint
    p30 = window_power(k_lo + 30, k_hi - 30, camp.FROZEN["v_hi"])
    assert abs(p30 - 0.911855) < 5e-7
    p46 = window_power(k_lo + 46, k_hi - 46, camp.FROZEN["v_hi"])
    ...
    assert 179_855 < expo < 179_857
    joint = p30 - 0.001 - 0.010 - math.exp(-700)   # exp(-expo) < 1e-300
    assert joint >= 0.900855 - 1e-6
...
def test_preflight_passes_only_on_the_exact_sha(monkeypatch, tmp_path):
    ...
    assert camp.preflight(_SHA)["seed"] == 40_000_451
...
def test_preflight_refuses_the_retired_stream_for_good(monkeypatch,
                                                       tmp_path):
    """With every other gate stubbed green, the REAL ledger is the
    one that now refuses: the observed stream has no fresh
    allocation, so no future checkout can run this campaign again."""
...
def test_the_retained_set_names_all_three_prior_claims():
    import o5_reservation as res
...
def _tiny(monkeypatch, tmp_path, ambiguous_every=0, out_every=0):
    ...
    small = dict(camp.FROZEN, a_intensity=64.0 / sz.SCALE, chunk=16,
...
def test_the_sprinkled_n_is_poisson_from_the_stream(monkeypatch,
    ...
    want = int(rng.poisson(small["a_intensity"] * sz.SCALE))
...
def test_a_race_losing_clean_refusal_files_no_incident(monkeypatch,
    ...
    def losing_claim(payload):
        raise SystemExit("reservation: refs/o5/reservation is "
...
def test_a_write_once_refusal_on_our_own_artifact_files_no_incident(
    ...
    monkeypatch.setattr(
        camp, "preflight",
        lambda rev: {"manifest": {}, "seed": 40_000_451,
...
def test_the_committed_manifest_is_the_tree_it_describes():
    ...
    for rel in ("docs/prereg/p14_o3p_volume.json",
...
def test_the_doc_freezes_the_discipline():
    doc = (_REPO / "docs" / "prereg"
           / "p14_o5_count.md").read_text(encoding="utf-8")
    flat = " ".join(doc.split())
    assert "no auto-raise, ever" in flat
    assert "Execution is NOT approved by this freeze" in flat
