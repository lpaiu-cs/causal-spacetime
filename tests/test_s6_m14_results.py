"""S6 rung M = 1.4 oracle-result contract tests.

The rung's certified volume was executed ONCE, from the clean exact
checkout of the freeze BRANCH HEAD `e5234e6` (the second parent of
merge `c25c1c8`, PR #91), under the frozen caps. These tests pin the
...
_FREEZE = "e5234e6ef7ad8fe91225fbde5857ab8b448e79bf"
_MERGE = "c25c1c8"
_ART = _REPO / "docs" / "prereg" / "p14_s6_m14_volume.json"
_SNAP = (_REPO / "docs" / "prereg"
         / "p14_s6_m14_executed_freeze_manifest.json")

sys.path.insert(0, str(_REPO / "experiments" / "oracle"))

import s6_m14_frozen_volume as run  # noqa: E402
...
def test_the_certified_interval_is_the_published_one():
    r = _artifact()["result"]
    assert r["v_lo"] == 64.33619774000562
    assert r["v_hi"] == 64.98274404119803
    assert r["calls"] == 506_766
...
def test_the_rung_identity_is_the_ladder_constant():
    art = _artifact()
    assert art["rung"]["m"] == 1.4
    assert art["rung"]["mu"] == s6.RUNG_CONSTANTS[1.4]["mu"]
    assert art["rung"]["scale"] == s6.RUNG_CONSTANTS[1.4]["scale"]
    assert art["frozen_config"]["dt"] == s6.RUNG_CONSTANTS[1.4]["dt"]
...
def test_executed_freeze_snapshot_matches_the_historical_blobs():
    hist = _blob("docs/prereg/p14_s6_m14_freeze_manifest.json")
