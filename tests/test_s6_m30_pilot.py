"""S6 M=3.0 ambiguity-pilot freeze contract tests.
...
def test_frozen_configuration_is_the_ruled_one():
    ...
    assert pilot.G3A_PREFLIGHT_CALLS == 6_466
...
def test_the_box_measure_identity_is_exact():
    ...
    assert g == {k: s6.rung_geometry(3.0)[k] for k in g}
...
def test_the_rule_is_general_in_k_and_matches_the_reference_table():
    ...
    expect = {
        0: (11.5092, 1.474909e-06, "FEASIBLE"),
        1: (16.5905, 9.999835e-04, "FEASIBLE"),
        2: (21.0081, 2.426026e-02, "INCONCLUSIVE"),
        3: (25.1047, 1.414943e-01, "INCONCLUSIVE"),
    }
...
def test_the_k1_boundary_margin_is_the_mpfr_determined_value():
    ...
    k=1 sits 1.6541e-08 of tail BELOW the budget. Pinned tightly --
    ...
    assert abs(margin - 1.6540586e-08) < 1e-15
...
def test_missing_freeze_rev_is_refused_by_the_cli():
    script = str(_REPO / "experiments" / "oracle" / "s6_m30_amb_pilot.py")
...
def test_preflight_passes_only_on_the_exact_sha(monkeypatch, tmp_path):
    ...
    assert pilot.preflight(_SHA)["seed"] == 40_000_501
...
def test_a_write_once_refusal_on_our_own_artifact_files_no_incident(
    ...
    monkeypatch.setattr(
        pilot, "preflight",
        lambda rev: {"manifest": {}, "seed": 40_000_501,
...
def test_the_doc_freezes_the_discipline():
    doc = (_REPO / "docs" / "prereg"
           / "p14_s6_m30_pilot.md").read_text(encoding="utf-8")
    flat = " ".join(doc.split())
...
def test_make_commit_refuses_cleanly_when_git_fails(monkeypatch):
    ...
    import s6_m30_reservation as res
